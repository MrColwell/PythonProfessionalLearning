...
***
Mr. Colwell Add-ons (Jan 2016):
.clear()        - clears the graphics window of objects
.getOverlap()   - returns tuple of overlapping objects
.getColor()     - returns current color of the object
***
...
class GraphicsObject:
    ...
    def getColor(self):
        #Added by Mr. Colwell
        #Allow user to check what color the object is
        colorAC=self.canvas.itemconfigure(self.id, 'fill')
        return colorAC[4]

    def getOverlap(self):
        #Added by Mr. Colwell
        #Allow user to check if another object is in the bounding box
        overlap=self.canvas.find_overlapping(self.p1.x,self.p1.y,
                                             self.p2.x,self.p2.y)
